...
def train(sess, env, args, actor, critic, actor_noise):
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(int(args['max_episode_len'])):
            ...
            a = actor.predict(np.reshape(s, (1, 75, 100, 1))) + actor_noise()
            #a = actor.predict(s) + actor_noise()
            while(abs(a[0]) > 0.25):
              a = actor.predict(np.reshape(s, (1, 75, 100, 1))) + actor_noise()

            #print('initialized value is smaller than 0.01')

            s2, r, terminal, info = env.step(a[0])

            replay_buffer.add(np.reshape(s, (75, 100, 1)), np.reshape(a, (actor.a_dim,)), r, terminal, np.reshape(s2, (75, 100, 1)))
            #replay_buffer.add(s, np.reshape(a, (actor.a_dim,)), r, terminal, s2)
...
def main(args):
    ...
    with tf.Session() as sess:
        ...
        action_bound = 0.1
        state_dim = 100*75 
        action_dim = 1 # speed is constant, wheele angle has 7 possibilities, +-0.025, +-0.05, +-0.1, 0
        #print(state_dim)
        #print(action_dim)
        #print(action_bound)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--buffer_size', help='max size of the replay buffer', default=1000)
    #parser.add_argument('--buffer_size', help='max size of the replay buffer', default=1000000)
    parser.add_argument('--minibatch_size', help='size of minibatch for minibatch-SGD', default=1)
    #parser.add_argument('--minibatch-size', help='size of minibatch for minibatch-SGD', default=64)

    # run parameters
    #parser.add_argument('--env', help='choose the gym env- tested on {Pendulum-v0}', default='Pendulum-v0')
    parser.add_argument('--env', help='choose the gym env- tested on {turtlebot-v0}', default='turtlebot-v0')
    ...
    parser.add_argument('--max_episode-len', help='max length of 1 episode', default=2000)
    ...
    parser.add_argument('--monitor_dir', help='directory for storing gym results', default='./results/gym_ddpg')
    parser.add_argument('--summary_dir', help='directory for storing tensorboard info', default='./results/tf_ddpg')
